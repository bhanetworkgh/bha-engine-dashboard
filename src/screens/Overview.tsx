...
function EventList({ title, events }: { title: string; events: OverviewEvent[] }) {
  return (
    <section className="flex min-h-0 min-w-0 flex-1 flex-col border-b border-line last:border-b-0 md:border-r md:border-b-0 md:last:border-r-0">
...
export default function Overview() {
  ...
  return (
    <div className="flex min-h-0 flex-col md:h-full">
      ...
      <div className="grid shrink-0 grid-cols-2 border-b border-line md:grid-cols-3 lg:grid-cols-5">
        {data.pins.map((p) => (
      ...
      <div className="grid shrink-0 grid-cols-1 border-b border-line md:grid-cols-3 lg:grid-cols-5">
        {data.tiles.map((t) => (
          <Link
            key={t.key}
            to={t.to}
