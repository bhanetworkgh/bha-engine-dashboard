...
export default function EngineHealth() {
  ...
  const tiles = data.systems.filter((s) => s.system !== 'all' && s.system !== 'unregistered');
  const unregistered = data.systems.find((s) => s.system === 'unregistered');
  const nothing = data.systems.every((s) => s.periods.every((p) => p.executions === 0));
  const worst = tiles
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="scroll-thin flex min-h-0 flex-1 flex-col overflow-x-hidden overflow-y-auto">
          ...
          <MetricCard
            ...
            note={
              <span className="block space-y-1">
                ...
                {data.source.at && <span className="block text-[11px] leading-snug text-faint">Last read {relativeTime(data.source.at) ?? data.source.at}.</span>}
              </span>
            }
          >
            {nothing ? (
              <p className="text-[12.5px] leading-relaxed text-dim">{data.source.note}</p>
            ) : (
                  ...
                  <Link key={s.system} to="/executions" className="flex items-start justify-between gap-3 rounded-[12px] bg-raised px-4 py-3 transition-colors hover:bg-hover">
                    <span className="min-w-0">
                      ...
                      <span className="mt-1 block text-[11px] leading-snug text-faint">
                        {s.executions ? `${s.failed} of ${s.executions} failed this week` : 'nothing ran this week'}
                      </span>
                    </span>
        ...
        */}
        {unregistered && unregistered.workflows.length > 0 && (
          <div className="mx-6 mb-4 md:mx-8">
            <MetricCard title="Workflows in no system" note="Each of these is running in n8n and has no row in the workflow registry naming its system, so on Executions it sits under Unregistered rather than under a system. Adding the registry row files it under the right one — a row, not a deploy.">
              <div className="space-y-1.5 text-[12.5px]">
                {unregistered.workflows.map((w) => (
                  <div key={w.workflow_id} className="flex items-baseline justify-between gap-3">
                    ...
                    <span className="tabular shrink-0 text-faint">
                      {w.executions} executions{w.failed ? <span className="text-failing"> · {w.failed} failed</span> : ''}
                    </span>
                  </div>
                ))}
              </div>
