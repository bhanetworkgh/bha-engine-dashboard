 ...
 * async and takes a Query, so wiring the live engine endpoint tomorrow is a
 * change to the bodies here — components already await, already handle loading
 * and failure, and already pass the lane filter through.
 *
 * Phase 2 replaces each body with a call to the engine endpoint. Nothing
 * outside this directory should need to change.
 */
...
export function getOverview(q: Query): Promise<OverviewData> {
  ...
  const totalOpen = q.lane === 'all' ? owners.reduce((n, o) => n + o.open, 0) : loops.filter((l) => l.status === 'open').length;
  const oldest = Math.max(...(loops.length ? loops.map((l) => l.age_days) : [0]));
  const entriesThisWeek = bySpineLane(f.CODEX_ENTRIES, q).filter((e) => e.week === '2026-W36').length;
    ...
    tiles: [
      {
        ...
        label: 'North Star',
        to: '/north-star',
        headline: String(bySpineLane(f.NS_RECORDS, q).length),
      ...
      },
      {
        ...
        label: 'Research Twin',
        to: '/research-twin',
        headline: String(bySpineLane(f.RT_RECORDS, q).length),
        sublabel: 'asks this period',
      ...
      },
      {
        ...
        signal: vfarmVisible ? 'lifecycle events not emitting' : 'filtered out',
        health: vfarmVisible && openAlerts.length ? 'degraded' : 'ok',
      },
      {
        key: 'engine-health',
      ...
      },
      {
        ...
        signal: `${bySpineLane(f.CODEX_ENTRIES, q).filter((e) => !e.ingested).length} posted, not ingested`,
        health: 'ok',
      },
      {
        key: 'build-patterns',
      ...
      },
      {
        ...
        label: 'Commercial',
        to: '/commercial',
        headline: String(byLane(f.OPPORTUNITIES, q).length),
        sublabel: 'opportunities',
        signal: `${byLane(f.OPPORTUNITIES, q).filter((o) => o.readiness === 'blocked').length} blocked`,
        health: byLane(f.OPPORTUNITIES, q).some((o) => o.readiness === 'blocked') ? 'failing' : 'ok',
      },
      {
        key: 'builders',
        label: 'Builders',
        to: '/builders',
        headline: String(byLane(f.BUILDERS, q).length),
        sublabel: 'people',
        signal: `${byLane(f.BUILDERS, q).filter((b) => b.contract_status !== 'signed').length} contract not signed`,
        health: byLane(f.BUILDERS, q).some((b) => b.contract_status !== 'signed') ? 'degraded' : 'ok',
      },
    ],
    broke_24h: [
      {
        id: 'B1',
        at: '11:42',
        title: 'Ask Cluster returning a wrapped 402',
        detail: 'BHARAG credit exhausted. No retry attempted — billing never auto-retries.',
        health: 'failing',
        spine: f.INCIDENTS[0].spine,
        source: f.INCIDENTS[0].source,
      },
      {
        id: 'B2',
        at: '09:15',
        title: 'Research Twin cannot resolve bharag2',
        detail: 'ENOTFOUND on two of three retries. Third pending.',
        health: 'degraded',
        spine: f.INCIDENTS[1].spine,
        source: f.INCIDENTS[1].source,
      },
      {
        id: 'B3',
        at: '13:48',
        title: 'pH above ceiling on rack-a/tier-3',
        detail: 'Held at 6.31 across three consecutive rollups.',
        health: 'degraded',
        spine: f.VFARM_ALERTS[0].spine,
        source: f.VFARM_ALERTS[0].source,
      },
      {
        id: 'B4',
        at: '09:06',
        title: 'Burn-in bench above temperature ceiling',
        detail: '23.1°C for 21 minutes against a 22.6°C ceiling.',
        health: 'degraded',
        spine: f.VFARM_ALERTS[1].spine,
        source: f.VFARM_ALERTS[1].source,
      },
      {
        id: 'B5',
        at: '08:14',
        title: 'Kaiqi digest disagreed with Kaiqi table',
        detail: 'Digest named three loops; the table returned none. Raised for reconciliation.',
        health: 'degraded',
        spine: f.LOOPS[f.LOOPS.length - 2].spine,
        source: f.LOOPS[f.LOOPS.length - 2].source,
      },
      {
        id: 'B6',
        at: '07:55',
        title: 'Commercial extractor hit its retry ceiling',
        detail: 'Three attempts, all failed. Classifier never derived a fix lane.',
        health: 'failing',
        spine: f.INCIDENTS[7].spine,
        source: f.INCIDENTS[7].source,
      },
      {
        id: 'B7',
        at: '06:20',
        title: 'Four Codex entries posted without ingest',
        detail: 'Reached Slack but not BHARAG, so no twin can cite them.',
        health: 'degraded',
        spine: f.CODEX_ENTRIES[3].spine,
        source: f.CODEX_ENTRIES[3].source,
      },
      {
        id: 'B8',
        at: '05:02',
        title: 'Founding-buyer card still blocked',
        detail: 'No authorised payment account. Unchanged for 31 days.',
        health: 'failing',
        spine: f.OPPORTUNITIES[0].spine,
        source: f.OPPORTUNITIES[0].source,
      },
      {
        id: 'B9',
        at: '02:41',
        title: 'Two loops named by the digest exist in no table',
        detail: 'Present in the Sept 7 digest, absent from all seven builder tables.',
        health: 'degraded',
        spine: f.LOOPS[16].spine,
        source: f.LOOPS[16].source,
      },
      {
        id: 'B10',
        at: '23:52',
        title: 'Ledger batch ingest rejected on schema',
        detail: 'incidents.v0 is strict; an added field fails the whole batch.',
        health: 'degraded',
        spine: f.INCIDENTS[2].spine,
        source: f.INCIDENTS[2].source,
      },
      {
        id: 'B11',
        at: '22:18',
        title: 'Research Twin quarantined an ask at three cycles',
        detail: 'Evidence stayed thin across three narrowing passes; handed to a human.',
        health: 'degraded',
        spine: f.RT_RECORDS[4].spine,
        source: f.RT_RECORDS[4].source,
      },
    ],
    moved_24h: [
      {
        id: 'M1',
        at: '14:02',
        title: 'North Star credential rotated',
        detail: 'INC-5A3C77 resolved after 26 hours of silent failure.',
        health: 'ok',
        spine: f.INCIDENTS[3].spine,
        source: f.INCIDENTS[3].source,
      },
      {
        id: 'M2',
        at: '11:20',
        title: 'Four blocking loops identified',
        detail: 'Bays narrowed 96 open loops to the 4 that block telemetry v1.',
        health: 'ok',
        spine: f.LOOPS[0].spine,
        source: f.LOOPS[0].source,
      },
      {
        id: 'M3',
        at: '10:07',
        title: 'Ledger corpora confirmed current',
        detail: 'vfarm.sensor and vfarm.alert both current, dead-letter queue at zero.',
        health: 'ok',
        spine: f.VFARM_ALERTS[2].spine,
        source: f.VFARM_ALERTS[2].source,
      },
      {
        id: 'M4',
        at: '08:17',
        title: 'Codex digest self-healed',
        detail: 'Empty render caught by assertion; second attempt posted.',
        health: 'ok',
        spine: f.INCIDENTS[4].spine,
        source: f.INCIDENTS[4].source,
      },
      {
        id: 'M5',
        at: '23:15',
        title: 'Three Codex entries ingested',
        detail: 'Destiny, Jegan and Kaiqi entries reached BHARAG rather than only posting.',
        health: 'ok',
        spine: f.CODEX_ENTRIES[0].spine,
        source: f.CODEX_ENTRIES[0].source,
      },
      {
        id: 'M6',
        at: '13:00',
        title: 'Per-builder loop digests delivered',
        detail: 'Seven builders, one digest each, prioritised by North Star.',
        health: 'ok',
        spine: f.LOOPS[5].spine,
        source: f.LOOPS[5].source,
      },
      {
        id: 'M7',
        at: '11:38',
        title: 'Ambient range question answered after going thin',
        detail: 'Two cycles. First transition recorded on the Research Twin gaps tab.',
        health: 'ok',
        spine: f.RT_TRANSITIONS[0].spine,
        source: f.RT_TRANSITIONS[0].source,
      },
      {
        id: 'M8',
        at: '10:30',
        title: 'pH by place confirmed present in the ledger',
        detail: 'Twenty-four hours of rollups readable across four places.',
        health: 'ok',
        spine: f.RT_RECORDS[0].spine,
        source: f.RT_RECORDS[0].source,
      },
      {
        id: 'M9',
        at: '09:20',
        title: 'Watched clients weekly clock completed',
        detail: 'Three client memos posted; contradiction state updated on Client 9.',
        health: 'ok',
        spine: f.INCIDENTS[6].spine,
        source: f.INCIDENTS[6].source,
      },
      {
        id: 'M10',
        at: '08:41',
        title: 'North Star answered both standing questions',
        detail: 'Top build lanes and where the Architect should look, both first cycle.',
        health: 'ok',
        spine: f.NS_RECORDS[1].spine,
        source: f.NS_RECORDS[1].source,
      },
      {
        id: 'M11',
        at: '21:10',
        title: 'Research Twin field rename mapped',
        detail: 'card_id to trace_id at the write node; queue writes green since.',
        health: 'ok',
        spine: f.INCIDENTS[5].spine,
        source: f.INCIDENTS[5].source,
      },
    ],
  });
}
...
export function getAskBays(_q: Query): Promise<AskBaysData> {
  return deliver({
    ...
    memory_note:
      'Bays holds no memory between threads today. History below is stored by this dashboard only, and is not yet wired to the agent.',
  });
}
...
export function getEngineHealth(q: Query): Promise<EngineHealthData> {
  ...
  return deliver({
    ...
    lanes_at_retry_ceiling: lanes
      .map((lane) => ({
        lane,
        incidents: incidents.filter(
          (i) => i.spine.lane === lane && i.retries_attempted >= i.max_retries,
        ).length,
      }))
